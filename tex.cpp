...
{                                                                                                                                               // TGA Loading code nehe.gamedev.net)
        ...
        texture_imageData       = (GLubyte *)calloc(tga.imageSize,1);                               // Allocate that much memory
...
GLubyte* GetTGAImageData(char *name)
{
        LoadTGA(name);
        char message[80];
        sprintf(message, "Size: %d" , tga.Height * tga.Width * tga.bytesPerPixel);
        MessageBox(NULL, message, "", NULL);
        return texture_imageData;

        texture_imageData = NULL;
}